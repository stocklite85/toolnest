...
export default function UrlEncoderPage() {
  ...
  const decode = () => {
    try {
      ...
      setError(isKo ? '유효하지 않은 인코딩 문자열입니다' : 'Invalid encoded string')
      setOutput('')
    }
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      ...
      <div className="mt-6 mb-6">
        <h1 className="text-3xl font-bold">{isKo ? 'URL 인코더 / 디코더' : 'URL Encoder / Decoder'}</h1>
        <p className="text-slate-400 mt-1 text-sm">
          {isKo ? 'URL과 쿼리 문자열을 인코딩하거나 디코딩하세요' : 'Encode and decode URLs and query strings'}
        </p>
      </div>
      ...
      <div className="space-y-4">
        <div>
          ...
          <textarea
            ...
            placeholder={isKo ? '인코딩할 텍스트 또는 디코딩할 URL 입력...' : 'Enter text or URL-encoded string...'}
            rows={5}
            className="w-full bg-slate-900 border border-slate-800 focus:border-rose-500/60 rounded-xl px-4 py-3 text-sm text-white placeholder-slate-600 outline-none transition-colors resize-none font-mono"
          />
        </div>

        <div className="flex gap-2 flex-wrap">
          <button onClick={encode}
            className="px-6 py-2.5 rounded-xl bg-rose-500 hover:bg-rose-400 active:scale-[0.98] text-white font-semibold text-sm transition-all">
            {isKo ? '인코딩' : 'Encode'}
          </button>
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            ...
            {output && (
              <button onClick={copy}
                className={`text-xs px-3 py-1 rounded-lg transition-all ${copied ? 'text-rose-400 bg-rose-500/10' : 'text-slate-500 hover:text-white'}`}>
